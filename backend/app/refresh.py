...
from .cross_division import rating_index, apply_cross_division_adjustment
from .reprice import reprice_game
...
def _board_card(result, game, home_row, away_row, market, provider_meta):
    return {
        ...
        "trench": {
            "away": {
                "RunMOTE": away_row.get("RunMOTE"),
                "PassMOTE": away_row.get("PassMOTE"),
                "RunOTE": away_row.get("RunOTE"),
                "PassOTE": away_row.get("PassOTE"),
            },
            "home": {
                "RunMOTE": home_row.get("RunMOTE"),
                "PassMOTE": home_row.get("PassMOTE"),
                "RunOTE": home_row.get("RunOTE"),
                "PassOTE": home_row.get("PassOTE"),
            },
        },
        "quality": {
...
def _classified_game_rows(cfbd, year, week):
    all_rows = []
    for cls in CLASSES:
        try:
            rows = cfbd.games(year, week, classification=cls)
        except Exception:
            rows = []
        for row in rows:
            r = dict(row)
            feeds = list(r.get("_te_classification_feeds") or [])
            if cls not in feeds:
                feeds.append(cls)
            r["_te_classification_feeds"] = feeds
            all_rows.append(r)

    # Merge duplicate cross-division games instead of dropping the classification feed metadata.
    merged = {}
    for row in all_rows:
        key = str(row.get("id"))
        if key not in merged:
            ...
            feeds = set(merged[key].get("_te_classification_feeds") or [])
            feeds.update(row.get("_te_classification_feeds") or [])
            merged[key]["_te_classification_feeds"] = sorted(feeds)
    return list(merged.values())

def _classified_advanced(cfbd, year, week=None):
    out = []
    for cls in CLASSES:
        try:
            rows = cfbd.advanced_game_stats(year, week=week, classification=cls)
        except Exception:
            rows = []
        for row in rows:
            p = advanced_to_partial_raw(row)
            p["classification"] = cls
            out.append(p)
    return _dedupe(out, lambda x: (x.get("game_id"), x.get("team")))

def _classified_havoc(cfbd, year, week=None):
    out = []
    for cls in CLASSES:
        try:
            rows = cfbd.game_havoc(year, week=week, classification=cls)
        except Exception:
            rows = []
        out.extend(rows)
    return _dedupe(out, lambda x: (str(x.get("gameId")), x.get("team")))

def _classified_plays(cfbd, year, week):
    out = []
    for cls in CLASSES:
        try:
            out.extend(cfbd.plays(year=year, week=week, classification=cls))
        except Exception:
            continue
    return _dedupe(out, lambda x: x.get("id") or (
        str(x.get("gameId")), x.get("period"), str(x.get("clock")), x.get("offense"), x.get("playText")
    ))

def _srs_rows(cfbd, year):
    rows = []
    for cls in CLASSES:
        try:
            rows.extend(cfbd.expanded_srs(year, classification=cls))
        except Exception:
            continue
    return _dedupe(rows, lambda x: x.get("team"))

def _class_for_team(team, ratings):
    return (ratings.get(team) or {}).get("classification")

def _build_class_aware_priors(slate, prev_rows, cur_rows):
    """Build priors inside each division so FCS missing values never inherit FBS global means."""
    out = []
    for cls in CLASSES:
        target = [r for r in slate if r.get("classification") == cls]
        if not target:
            continue
        prev = [r for r in prev_rows if r.get("classification") == cls]
        cur = [r for r in cur_rows if r.get("classification") == cls]
        out.extend(build_pregame_rows(target, prev, cur))

    # Unknown classification is still allowed, but isolated from FBS/FCS global means.
    unknown = [r for r in slate if r.get("classification") not in CLASSES]
    if unknown:
        out.extend(build_pregame_rows(unknown, prev_rows, cur_rows))
    return out

def _standardize_by_class(rows, fields):
    from .standardize import standardize_slate
    for cls in CLASSES:
        group = [r for r in rows if r.get("classification") == cls]
        if group:
            standardize_slate(group, fields)

    unknown = [r for r in rows if r.get("classification") not in CLASSES]
    if unknown:
        standardize_slate(unknown, fields)
...
def refresh_week(year:int=2026, week:int=2) -> Dict[str,Any]:
    ...
    games = _classified_game_rows(cfbd, year, week)
    if not games:
        return {"games":[],"refresh_status":"NO_GAMES"}

    # SRS expanded is deliberately used because it contains both FBS and FCS teams.
    # Fall back to prior season if the current early-season table is incomplete.
    srs_rows = _srs_rows(cfbd, year)
    srs = rating_index(srs_rows)
    if len(srs) < 50:
        prior_srs = rating_index(_srs_rows(cfbd, year-1))
        prior_srs.update(srs)
        srs = prior_srs

    previous_partials = _classified_advanced(cfbd, year-1)
    current_partials = _classified_advanced(cfbd, year, week=max(1,week-1))
    previous_havoc = _classified_havoc(cfbd, year-1)
    current_havoc = _classified_havoc(cfbd, year, week=max(1,week-1))

    merge_havoc(previous_partials, previous_havoc)
    merge_havoc(current_partials, current_havoc)

    prev_plays = []
    for w in range(1,16):
        prev_plays.extend(_classified_plays(cfbd, year-1, w))

    cur_plays = []
    for w in range(1, max(1,week)):
        cur_plays.extend(_classified_plays(cfbd, year, w))

    attach_sack_rates(previous_partials, prev_plays)
    attach_sack_rates(current_partials, cur_plays)

    prev_rows = [_team_row_from_partial(p) for p in previous_partials]
    cur_rows = [_team_row_from_partial(p) for p in current_partials]
    ...
    for g in games:
        ...
        slate.append({
            ...
            "team": home,
            "opponent": away,
            "classification": _class_for_team(home, srs),
        })
        slate.append({
            "game_id": str(g.get("id")),
            "team": away,
            "opponent": home,
            "classification": _class_for_team(away, srs),
        })

    priors = _build_class_aware_priors(slate, prev_rows, cur_rows)
    ...
    _standardize_by_class(priors, standard_fields)
    build_mote_slate(priors)

    by_game = defaultdict(dict)
    for r in priors:
        by_game[str(r["game_id"])][r["team"]] = r
    ...
    for g in games:
        ...
        home = g.get("homeTeam")
        away = g.get("awayTeam")
        if home not in by_game[gid] or away not in by_game[gid]:
            continue
        ...
        dfr_fields = [
            "pre_off_epa","pre_off_pass_epa","pre_off_rush_epa",
            "pre_off_firstdown","pre_off_pass_protect","pre_off_havoc_avoid",
            "RunMOTE","PassMOTE","Reliability"
        ]
        dfr_inputs_h = {k:hr.get(k) for k in dfr_fields}
        dfr_inputs_a = {k:ar.get(k) for k in dfr_fields}

        fpse_neutral = {
            "st_epa_pp_pre":0.0,"punt_epa_pp_pre":0.0,"ko_epa_pp_pre":0.0,
            "havoc_total_rate_pre":0.12,"takeaway_rate_pre":0.02,
            "Reliability":hr.get("Reliability",0)
        }
        dfr_h = calculate_dfr(dfr_inputs_h)
        dfr_a = calculate_dfr(dfr_inputs_a)
        fpse_h = calculate_fpse(fpse_neutral)
        fpse_a = calculate_fpse(fpse_neutral)
        ...
        dqh,_ = calculate_dqs(dqs_h.model_dump())
        dqa,_ = calculate_dqs(dqs_a.model_dump())
        oh,_ = calculate_ocrs(ocrs_h.model_dump())
        oa,_ = calculate_ocrs(ocrs_a.model_dump())
        ...
        req = MatchupRequest(
            ...
            derived_metrics=DerivedMetrics(
                ...
                ocrs=max(oh,oa),
                dfr=max(dfr_h,dfr_a),
                fpse=max(fpse_h,fpse_a),
                source="refresh_v6_class_aware",
                details={
                    "home_dqs":dqh,"away_dqs":dqa,
                    "home_ocrs":oh,"away_ocrs":oa
                }
            ),
        )
        ...
        try:
            home_meta = cfbd.team(home)
        except Exception:
            home_meta = None
        try:
            away_meta = cfbd.team(away)
        except Exception:
            away_meta = None

        card = _board_card(result,g,hr,ar,market,{
            "cfbd":True,
            "odds":odds.ready,
            ...
            "week":week,
            "classifications":{
                "home":_class_for_team(home,srs),
                "away":_class_for_team(away,srs),
            },
            "branding":{
        ...
        # Locked V4.1 stays untouched. The normalization is an explicit post-model layer.
        apply_cross_division_adjustment(card, srs)
        reprice_game(card, market)
        board.append(card)
    ...
    payload = {
        ...
        "year":year,
        "week":week,
        "model_refreshed_epoch":finished,
        "model_refresh_duration_seconds":round(finished-started,2),
        "pipeline_version":"v6_live_recompute_cross_division",
        "cross_division_normalization":"expanded_srs_then_historical_class_prior",
    }
    (DATA_DIR/f"week{week}_outputs.json").write_text(json.dumps(payload,indent=2))
    # Retained for compatibility with older frontend/builds.
