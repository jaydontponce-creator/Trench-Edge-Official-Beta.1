...
function scoreboardCard(g){
 ...
 return `<div class=score-game>
  ...
  <div class=score-team><div class=helmet style="background:${c.away};color:${c.awaySecondary||"#fff"}"></div><div><div class=score-team-name>${g.away}</div><div class=score-team-record>${teamDivision(g.away)}</div></div><div class=score-number>${g.away_score??"–"}</div></div>
  <div class=score-team><div class=helmet style="background:${c.home};color:${c.homeSecondary||"#fff"}"></div><div><div class=score-team-name>${g.home}</div><div class=score-team-record>${teamDivision(g.home)}</div></div><div class=score-number>${g.home_score??"–"}</div></div>
  <div class=score-status><span>${live?'<span class=live-dot></span>LIVE':status}</span><span class=game-clock data-clock="${g.clock||""}" data-period="${g.period||""}">${g.clock||g.detail||""}</span></div>
 </div>`;
}
...
$("divisionToggle").onchange=e=>{state.division=e.target.value;render();};
