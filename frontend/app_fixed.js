...
function inlineDetail(g){
  ...
  return `<div class="game-detail-inline" onclick="event.stopPropagation()">
    ...
    <div class=tabs>
      ...
      ${tab("why","Why TE Likes It")}
      ${tab("risk","Risk Factors")}
...
function render(){
  ...
  $("games").innerHTML=a.length?a.map(g=>{
    ...
    return `<div class="game ${expanded?"expanded":""}" data-id="${g.id}">
      <div class=row>
        <div>
          <div class=match>${g.away} @ ${g.home}</div>
          <div class=small>${market(g)}</div>
        </div>
        <span class="badge ${g.decision?.state||"NEEDS_DATA"}">${verdict(g)}</span>
      </div>
      <div class="row small" style="margin-top:8px">
        <span>${g.prediction?.away_points==null?"Analysis pending":`Projected ${g.away} ${whole(g.prediction.away_points)}, ${g.home} ${whole(g.prediction.home_points)}`}</span>
        <span>${g.decision?.edge==null?"":`${one(Math.abs(g.decision.edge))} pt value`}</span>
      </div>
      <div class=expand-cue>${expanded?"Click matchup to collapse":"Click matchup to open full prediction"}</div>
      ${expanded?inlineDetail(g):""}
    </div>`;
  }).join(""):`<div class=panel>No games match this filter.</div>`;

  document.querySelectorAll(".game").forEach(el=>el.onclick=(evt)=>{
    if(evt.target.closest(".tab") || evt.target.closest(".game-detail-inline")) return;
  ...
  document.querySelectorAll(".tab").forEach(el=>el.onclick=(evt)=>{
    evt.stopPropagation();
